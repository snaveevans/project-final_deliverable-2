...
use Notes\Persistence\Entity\PermissionGroup\Role;

describe('Notes\Persistence\Entity\PermissionGroup\Role', function () {
    describe('->__construct()', function () {
        it('should construct an Group object', function () {
            $uuid = new Uuid();
            $role = new Role($uuid);
            expect($role)->to->be->instanceof('Notes\Persistence\Entity\Role');
        }) ;
    });
    describe('->getId()', function () {
        it('should return Id of Role', function () {
            $uuid = new Uuid();
            $role = new Role($uuid);
            expect($role->getId())->to->be->instanceof('Notes\Persistence\Entity\Uuid');
        });
    });
